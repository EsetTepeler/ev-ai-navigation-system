...
class RoutePlanningService:
            ...
            if response.status_code == 200:
                ...
                if "routes" in data and len(data["routes"]) > 0:
                    ...
                    return {
                        ...
                        "traffic_delay_minutes": summary.get("trafficDelayInSeconds", 0)
                        / 60,
    ...
    def calculate_charging_stops(
        ...
        try:
            ...
            logger.info(
                f"Route planning: {total_distance:.1f}km, Range: {vehicle_range_km}km, Usable: {usable_range:.1f}km"
            )
            ...
            if not all_stations:
                ...
                tomtom_data = self.get_tomtom_route(
                    start_lat, start_lon, end_lat, end_lon
                )
            ...
            charging_stops = self._find_optimal_stops(
                ...
                min_charge_percent,
                preferred_charge_percent,
            )

            # Get TomTom route with waypoints for charging stops
            waypoints = [
                (stop["latitude"], stop["longitude"]) for stop in charging_stops
            ]
            tomtom_data = self.get_tomtom_route(
    ...
    def _find_optimal_stops(
        ...
        while True:
            segment_number += 1
            distance_to_end = self.haversine_distance(
                current_lat, current_lon, end_lat, end_lon
            )

            # Calculate remaining range with current battery
            remaining_range = (
                vehicle_range_km * (current_battery - min_charge_percent) / 100
            )

            # If we can reach destination with current battery, we're done
            if distance_to_end <= remaining_range:
                logger.info(
                    f"Segment {segment_number}: Can reach destination ({distance_to_end:.1f}km < {remaining_range:.1f}km)"
                )
                break
            ...
            for station in stations:
                ...
                dist_from_current = self.haversine_distance(
                    current_lat, current_lon, station["latitude"], station["longitude"]
                )
                ...
                range_after_charge = (
                    vehicle_range_km
                    * (preferred_charge_percent - min_charge_percent)
                    / 100
                )
                if (
                    dist_to_destination > range_after_charge * 1.2
                ):  # Still too far even after charging
            ...
            if not candidate_stations:
                logger.warning(
                    f"No reachable charging stations found at segment {segment_number}"
                )
                break
            ...
            kwh_needed = (
                battery_capacity_kwh
                * (preferred_charge_percent - current_battery)
                / 100
            )
            ...
            stop_info = {
                ...
                "distance_from_start": sum(s["distance_traveled"] for s in stops)
                + km_to_station,
    ...
    def _create_route_response(
        ...
        total_charging_minutes = sum(
            stop["charging_time_minutes"] for stop in charging_stops
        )
        ...
        return {
            "success": True,
            "message": message
            or "Route calculated successfully with real-time traffic",
            "route_summary": {
                ...
                "estimated_cost_tl": round(
                    total_charging_cost * 30, 2
                ),  # Convert to TL
                "energy_needed_kwh": round(
                    sum(stop["kwh_charged"] for stop in charging_stops), 2
                ),
