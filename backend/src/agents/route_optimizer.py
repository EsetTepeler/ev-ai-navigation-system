...
from models.navigation import RouteResponse, Waypoint, ChargingStop, WaypointType
...
class RouteOptimizerAgent:
    ...
    async def optimize_route(
        ...
        vehicle_specs: Dict[
            str, Any
        ],  # Accept dict instead of VehicleSpecs for flexibility
        ...
        try:
            logger.info(
                f"Optimizing route for {vehicle_specs.get('model', 'unknown vehicle')}"
            )

            # 1. Calculate base route without charging stops
            base_route = await self.navigation_service.calculate_base_route(
                start, destination
            )
            ...
            logger.info(
                f"Route optimization completed: {len(charging_stops)} charging stops"
            )
    ...
    async def _analyze_energy_requirements(
        ...
        consumption_rate = vehicle_specs.get(
            "energy_consumption_kwh_per_100km", 20
        )  # Default 20 kWh/100km
        ...
        # Available energy
        battery_capacity = vehicle_specs.get(
        ...
        return {
            ...
            "estimated_final_battery": max(
                0, (current_energy_kwh - total_energy_needed) / battery_capacity
            ),
        }
    ...
    async def _plan_charging_stops(
        ...
        for i, segment in enumerate(route_segments):
            ...
            energy_needed = segment_energy + (
                battery_capacity * self.SAFETY_BUFFER_PERCENTAGE
            )
            ...
            if available_energy < energy_needed:
                ...
                charging_station = (
                    await self.charging_service.find_best_charging_station(
                ...
                if charging_station:
                    ...
                    charging_stop = ChargingStop(
                        ...
                        estimated_cost=charging_station["cost_per_kwh"]
                        * charge_needed
                        * battery_capacity,
    ...
    def _calculate_segment_energy(
        self, segment: Dict[str, Any], vehicle_specs: Dict[str, Any]
    ) -> float:
        ...
        elevation_factor = 1.0 + (
            segment.get("elevation_gain_m", 0) / 1000 * 0.1
        )  # 10% per km elevation
        speed_factor = 1.0 + (
            max(0, segment.get("avg_speed_kmh", 80) - 80) / 100 * 0.1
        )  # Highway penalty
    ...
    async def _create_optimized_waypoints(
        ...
        for i, segment in enumerate(base_route["segments"]):
            ...
            if charging_stop_index < len(
                charging_stops
            ) and self._is_charging_stop_near_segment(
                charging_stops[charging_stop_index], segment
            ):
    ...
    def _is_charging_stop_near_segment(
        self, charging_stop: ChargingStop, segment: Dict[str, Any]
    ) -> bool:
        ...
        distance = (
            math.sqrt((stop_lat - seg_lat) ** 2 + (stop_lon - seg_lon) ** 2) * 111
        )  # Rough km conversion
    ...
    async def _calculate_route_metrics(
        ...
        total_time = sum(
            [
                w.estimated_charging_time or 0
                for w in waypoints
                if w.waypoint_type == "charging"
            ]
        )
