...
from data.car_models.ev_models import FLAT_CAR_MODELS, get_car_by_id
...
async def get_all_vehicles(
    ...
    try:
        ...
        if manufacturer:
            vehicles = [
                v for v in vehicles if v["manufacturer"].lower() == manufacturer.lower()
            ]
        ...
        for vehicle in vehicles:
            model = VehicleModel(
                ...
                energy_consumption_kwh_per_100km=vehicle[
...
async def get_manufacturers():
    ...
    return {"manufacturers": sorted(manufacturers), "count": len(manufacturers)}
...
async def get_vehicle_by_id(vehicle_id: str):
    ...
    try:
        ...
        vehicle = VehicleModel(
            ...
            energy_consumption_kwh_per_100km=vehicle_data[
                "energy_consumption_kwh_per_100km"
            ],
...
async def compare_vehicles(vehicle_ids: List[str]):
    ...
    try:
        if len(vehicle_ids) > 5:
            raise HTTPException(
                status_code=400, detail="Maximum 5 vehicles can be compared"
            )
        ...
        comparison_metrics = {
            ...
            "efficiency_winner": min(
                vehicles, key=lambda x: x.energy_consumption_kwh_per_100km
            ).model,
            "value_winner": min(vehicles, key=lambda x: x.price_usd / x.range_km).model,
        }

        return VehicleComparison(
            vehicles=vehicles, comparison_metrics=comparison_metrics
        )
...
async def get_recommendations_by_use_case(
    use_case: str = Query(
        ...,
        description="Use case: daily_commute, long_distance, family, luxury, budget",
    ),
    ...
    try:
        ...
        if use_case == "daily_commute":
            ...
            candidates = sorted(
                all_vehicles, key=lambda x: x["energy_consumption_kwh_per_100km"]
            )
            ...
            candidates = sorted(
                all_vehicles,
                key=lambda x: (-x["range_km"], -(x.get("fast_charge_power_kw") or 0)),
            )
            recommendations = candidates[:3]
