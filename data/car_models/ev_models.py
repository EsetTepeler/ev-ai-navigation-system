...
def safe_int(value, default=0):
    """Safely convert value to int, handling strings with text"""
    if not value:
        return default
    try:
        ...
        with open(csv_path, "r", encoding="utf-8") as f:
            ...
            for idx, row in enumerate(reader, 1):
                try:
                    ...
                    range_km = safe_int(row.get("range_km"))
                    ...
                    vehicle = {
                        ...
                        "fast_charge_power_kw": safe_int(
                            row.get("fast_charge_power_kW")
                        ),
                        "fast_charge_time_min": safe_int(
                            row.get("fast_charge_time_min")
                        ),
                        "acceleration_0_100_kmh": float(
                            row.get("acceleration_0_100_s", 0) or 0
                        )
                        if row.get("acceleration_0_100_s")
                        else None,
                        "supported_connectors": connectors,
                        "top_speed_kmh": safe_int(row.get("top_speed_kmh")),
                        "price_usd": price_usd,
                        "drivetrain": row.get("drivetrain", "FWD"),
                        "body_type": row.get("car_body_type", "Sedan"),
                        "seats": safe_int(row.get("seats"), 5),
                        "cargo_volume_l": safe_int(row.get("cargo_volume_l")),
...
# Print summary
print(
    f"EV Models loaded: {len(FLAT_CAR_MODELS)} vehicles from {len(ALL_CAR_MODELS)} manufacturers"
)
